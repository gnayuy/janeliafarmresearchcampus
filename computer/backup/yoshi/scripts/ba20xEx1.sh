...
SUBGECT=/groups/scicomp/jacsData/yuTest/YoshiMB/sources/flybrainTX.v3draw

#STR=`echo $i | awk -F\. '{print $1}'`
#CNT=$(echo $STR | tr -cd '[[:digit:]]')

qsub -b y -cwd -V -pe batch 8 -l mem96=true /groups/scicomp/jacsData/yuTest/YoshiMB/scripts/brainalign20x.sh $TARGET $TARGET_REFNO $TARGET_LANDMARKS $SUBGECT $SUBJECT_REFNO $OUTPUT $TEMPLATE
